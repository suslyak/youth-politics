...
import ThumbsRenderer from './ThumbsRenderer';
...
class Viewer extends PureComponent {
    ...
    recomputeRowHeights() {
        ...
        this._thumbsList.current.resetAfterIndex(0);
    }
    ...
    computeRowHeight(index) {
        ...
        if (cachedPageDimensions && responsiveScale) {
            return (cachedPageDimensions.get(index + 1)[1] / responsiveScale) + 5;
        }

        return 768; // Initial height
    }

    computeThumbsRowHeight(index) {
        const { cachedPageDimensions, responsiveScale } = this.state;

        if (cachedPageDimensions && responsiveScale) {
            return (cachedPageDimensions.get(index + 1)[1] / (responsiveScale * 4)) + 50;
        }
    ...
    updateCurrentVisiblePage({ visibleStopIndex }) {
        ...
        this._thumbsList.current.scrollToItem(visibleStopIndex + 1);

        const pagesControlInput = document.querySelector(`input[name='current-page-control']`);

        pagesControlInput.value = visibleStopIndex + 1;
    }
    ...
    handlePageInputChange(page) {
        this._list.current.scrollToItem(page);
    }
    ...
    render() {
        ...
        return (
            <Document
                file="files/UN/UN0142.pdf"
                loading={<Loader />}
                onLoadSuccess={this.onDocumentLoadSuccess.bind(this)}
                onLoadError={(error) => console.error(error)} // eslint-disable-line no-console
            >
                {cachedPageDimensions && (<>
                    <div className="book-reader__thumbs-div">
                        <VariableSizeList
                            height={containerHeight}
                            itemCount={pdf.numPages}
                            itemSize={this.computeThumbsRowHeight.bind(this)}
                            itemData={{
                                scale,
                                pages,
                                pageNumbers,
                            ...
                            overscanCount={2}
                            onItemsRendered={() => false}
                            ref={this._thumbsList}
                        >
                            {ThumbsRenderer}
                        </VariableSizeList>
                    </div>
                    <div className="book-reader__pages-div">
                        <PagesControls
                            currentPage={this.state.currentPage}
